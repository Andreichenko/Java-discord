...
public class SeekCommand extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        if (!audioManager.isConnected()){
            ...
            event.getChannel().sendMessage(TextChannelResponse.BOT_NOT_CONNECTED_TO_VOICE).queue();
            return;
        }
        ...
        try {
            ...
            event.getChannel().sendMessage(TextChannelResponse.SEEK_COMMAND_FORMAT).queue();
            return;
        }

        if (seekTime * 1000 > audioTrack.getDuration() || !audioTrack.isSeekable()){
            event.getChannel().sendMessage(TextChannelResponse.SEEK_POINT_LONGER_THAN_SONG).queue();
            return;
        }
        event.getChannel().sendMessage(String.format(TextChannelResponse.SEEKING_TO_INFORMATION,
