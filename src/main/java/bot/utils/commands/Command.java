...
public abstract class Command {
    ...
    /**
     * {@code true} if the command can be used in a {@link net.dv8tion.jda.api.entities.Guild Guild},
     * {@code false} if it can't be used in a guild.
     * <br>Default {@code true}.
     */
    protected boolean allowedInGuild = true;

    /**
     * {@code true} if the command can be used in a DM,
     * {@code false} if it can't be used in a dm.
     * <br>Default {@code false}.
     */
    protected boolean allowedInDm = false;
    ...
    public final void run(CommandEvent event) {
        ...
        if (requiredRole != null)
            if (!event.isFromType(ChannelType.TEXT) || event.getMember().getRoles().stream().noneMatch(r -> r.getName().equalsIgnoreCase(requiredRole))) {
                terminate(event, event.getClient().getError() + " You must have a role called `" + requiredRole + "` to " +
                        "use that!");
                return;
            }
        ...
        if (event.getChannelType() == ChannelType.TEXT) {
            if (!allowedInGuild) {
                if (!hidden) {
                    terminate(event, event.getClient().getError() + " This command cannot be used in Guilds");
                }
                return;
            }
                ...
                if (p.isChannel()) {
                    if (p.name().startsWith("VOICE")) {
                        ...
                        if (vc == null) {
                            ...
                            terminate(event, String.format(BOT_PERM, event.getClient().getError(), p.getName(), "Voice " +
                                    "Channel"));
                            return;
                        }
                    }
    ...
    /**
     * Checks if this command can be used in a guild
     *
     * @return true if this can be used in a guild
     */
    public boolean isAllowedInGuild() {
        return allowedInGuild;
    }

    /**
     * Checks if this command can be used in a DM
     *
     * @return true if this can be used in a DM
     */
    ...
    public String getCooldownKey(CommandEvent event) {
        switch (cooldownScope) {
            ...
            case USER_GUILD:
                return event.getGuild() != null ? cooldownScope.genKey(name, event.getAuthor().getIdLong(),
            ...
            case SHARD:
                return event.getJDA().getShardInfo() != null ? cooldownScope.genKey(name,
                        event.getJDA().getShardInfo().getShardId()) :
                        CooldownScope.GLOBAL.genKey(name, 0);
            case USER_SHARD:
                return event.getJDA().getShardInfo() != null ? cooldownScope.genKey(name, event.getAuthor().getIdLong(),
                        event.getJDA().getShardInfo().getShardId()) :
    ...
    public enum CooldownScope {
         ...
         * </ul>
         *
         * <p><b>NOTE:</b> This will automatically default back to {@link Command.CooldownScope#CHANNEL CooldownScope
         * .CHANNEL}
